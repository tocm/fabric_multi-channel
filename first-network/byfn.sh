...
function networkUp() {
  ...
  if [ ! -d "crypto-config" ]; then
    echo "=========> to do generate crypto-config..."
    generateCerts
    replacePrivateKey
...
function generateChannelArtifacts() {
 ...
 # configtxgen -profile TwoOrgsChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel one  configuration transaction..."
    exit 1
  fi
  set -x
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel two configuration transaction..."
    exit 1
  fi
  echo
  echo "#################################################################"
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ${CHANNEL_ONE_PROFILE} -outputAnchorPeersUpdate \
    ./channel-artifacts/Org2MSPanchors_${CHANNEL_ONE_NAME}.tx -channelID $CHANNEL_ONE_NAME -asOrg Org2MSP
...
ADDITIONS=""
  ...
  if [ "${IF_CAS}" == "1" ]; then
    ADDITIONS="${ADDITIONS} and using Fabric CAs support hy-composer author by Andy.chen"
fi
